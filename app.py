...
import os
import simulatedannealing as algr
...
class App(Frame):
	...
	def initUI(self):
		...
		self.lbNumOfInteractions = Label(self.frameWidget, text = "Num of Interactions")

		# Tham số pady = 25, để label cách đều về 2 phía theo trục y 25 px
		self.lbNumOfInteractions.pack(pady = 25)

		# Tạo textbox để nhập số lần lập của thuật toán

		self.entryNumOfInteractions= Entry(self.frameWidget)
		self.entryNumOfInteractions.pack()

		# Tạo button để chạy giải quyết thuậ toán
		# Tham số command = self.resolving để tạo sự kiện khi nhấn button này
		self.btnResolving = Button(self.frameWidget, text = "Simulated Annealing", command = self.resolving)
	...
	def resolving(self):
		...
		temperature = int(self.entryNumOfInteractions.get())

		solution, ti = algr.getResults(numOfQueens,temperature)
